...
class ChargePoint(cp):
    ...
    def __init__(
        ...
        self.tasks = None
        self._last_recorded_transactions: dict[int, int | str | None] = {}
    ...
    def _record_session_energy_usage(self, connector_id: int = 0) -> None:
        ...
        session_energy = self._metrics[(connector_id, csess.session_energy.value)].value
        id_tag = self._metrics[(connector_id, cstat.id_tag.value)].value
        transaction_id = self._metrics[(connector_id, csess.transaction_id.value)].value

        if not id_tag or session_energy is None:
            return

        if transaction_id is not None and transaction_id == self._last_recorded_transactions.get(
            connector_id
        ):
            return

        try:
            energy_value = float(session_energy)
        ...
        central.record_tag_energy(id_tag, energy_value)
        if transaction_id is not None:
            self._last_recorded_transactions[connector_id] = transaction_id
